...
const PlaceCard = ({
  ...
  const handleClick = () => {
  const existing = localStorage.getItem("clickedPlaceIds");
  const placeIds: number[] = existing ? JSON.parse(existing) : [];

  if (!placeIds.includes(id)) {
    placeIds.push(id);
    localStorage.setItem("clickedPlaceIds", JSON.stringify(placeIds));
  }
